...
void Socket::BindAddr(struct sockaddr_in& addr, uint32 port, const char* ip)
{
	m_port = port;
...
bool Socket::Listen(uint32 port)
{
	BindAddr(m_sockAddr, port);
	sint32 ret = bind(m_fd, (const sockaddr*)&m_sockAddr, sizeof(m_sockAddr));
	if (ret < 0)
	{
		return false;
	}
...
bool Socket::Connect(const char* ip, uint32 port)
{
...
void Socket::SetKeepAlive(uint32 interval)
{
	int val = 1;
